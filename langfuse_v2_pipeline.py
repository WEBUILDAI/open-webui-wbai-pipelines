"""
title: Langfuse v2 Filter Pipeline
author: open-webui
date: 2025-06-16
version: 1.0.0
license: MIT
description: A filter pipeline that uses Langfuse v2.x.
requirements: langfuse>=2.0.0,<3.0.0
"""
...
class Pipeline:
    ...
    async def on_startup(self):
        """Initialize Langfuse client when the service starts."""
        self.log(f"on_startup triggered for {__name__}")
        self.set_langfuse()

    async def on_shutdown(self):
        """Flush pending data when the service shuts down."""
        self.log(f"on_shutdown triggered for {__name__}")
        if self.langfuse:
            self.langfuse.flush()

    async def on_valves_updated(self):
        """Reset Langfuse client when valves are updated."""
        self.log("Valves updated, resetting Langfuse client.")
        self.set_langfuse()

    def set_langfuse(self):
        """Initialize the Langfuse client with provided credentials."""
        try:
            ...
            # In Langfuse v2, we don't have direct auth_check method, 
            # so we'll create a simple trace to verify connectivity
            test_trace = self.langfuse.trace(
                name="auth-check",
                metadata={"test": True}
            )
            if test_trace and test_trace.id:
                self.log("Langfuse client initialized successfully.")
            else:
                print("Failed to initialize Langfuse client. Please check your credentials.")
    ...
    async def inlet(self, body: dict, user: Optional[dict] = None) -> dict:
        ...
        if chat_id not in self.chat_traces:
            ...
            # In Langfuse v2, we use the trace method directly with keyword arguments
            ...
            if self.valves.debug and trace:
                print(f"[DEBUG] Created Langfuse trace with ID: {trace.id}")

            self.chat_traces[chat_id] = trace
        else:
            trace = self.chat_traces[chat_id]
            self.log(f"Reusing existing trace for chat_id: {chat_id}")
            # In Langfuse v2, we can update tags this way
            if tags_list:
        ...
        if task_name in self.GENERATION_TASKS:
            ...
            # In Langfuse v2, the generation API is different
            generation = self.langfuse.generation(
                name=f"{task_name}:{str(uuid.uuid4())}",
                trace_id=trace.id if trace else None,
                model=model_value,
                model_parameters={},  # Add model parameters if available
                input=body["messages"],
                metadata=metadata,
                tags=tags_list if tags_list else None
            )

            if self.valves.debug and generation:
                print(f"[DEBUG] Created Langfuse generation with ID: {generation.id}")
        else:
            # Otherwise, log it as an event
            # In Langfuse v2, the event API is different
            ...
            if self.valves.debug and event:
                print(f"[DEBUG] Created Langfuse event with ID: {event.id}")
    ...
    async def outlet(self, body: dict, user: Optional[dict] = None) -> dict:
        ...
        if task_name in self.GENERATION_TASKS:
            ...
            # If usage information exists, add it to the metadata
            if usage:
                metadata["usage"] = usage

            # In Langfuse v2, we create a new generation and mark it as ended in one step
            generation = self.langfuse.generation(
                name=f"{task_name}:{str(uuid.uuid4())}",
                trace_id=trace.id if trace else None,
                model=model_value,
                ...
                metadata=metadata,
                tags=tags_list if tags_list else None,
                # For v2, usage has a different structure - include token counts directly
                prompt_tokens=usage["prompt_tokens"] if usage else None,
                completion_tokens=usage["completion_tokens"] if usage else None,
                total_tokens=usage["total_tokens"] if usage else None,
                # Mark as ended
                completion_start_time=None,  # Optional, if you track start time
                end_time=None,  # Optional, if you track end time
            )

            if self.valves.debug and generation:
                print(f"[DEBUG] Created completed Langfuse generation with ID: {generation.id}")
            self.log(f"Generation completed for chat_id: {chat_id}")
        else:
            # Otherwise log as an event
            # In Langfuse v2, we use the event method directly
